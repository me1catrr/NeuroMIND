#!/usr/bin/env julia
# ═══════════════════════════════════════════════════════════════
#  NeuroMIND — Visor interactivo PSD espectral (canal + promedio)
# ═══════════════════════════════════════════════════════════════
#
#  Compara el PSD de un canal con el PSD promedio y muestra potencia
#  por banda + índices espectrales.
#
#  ⚠  No es señal cruda: el PSD sale del paso [6/8] del pipeline
#     (filtrado → ICA → segmentación → AR → FFT Hamming), guardado en:
#       tables/psd_by_channel.csv      ← curvas del plot
...
#
#  Salida:
#    psd_<canal>_fmax<N>.png
#
#  Uso:
#    julia --project=. src/interactive/aux/sub-M05_ses-T2_eyesclosed/plot_spectral_PSD.jl
#    # → http://127.0.0.1:8767/  (Ctrl+C para salir)
#
# ───────────────────────────────────────────────────────────────
#  Fichero     src/interactive/aux/sub-M05_ses-T2_eyesclosed/plot_spectral_PSD.jl
...
function load_store()::PSDStore
    ...
    return PSDStore(channels, freqs, psd, mean_psd, bp, idx, reg, FS_HZ)
end

function _band_row(store::PSDStore, channel::String)::Dict{String,Float64}
...
function _mean_band_power(store::PSDStore)::Dict{String,Float64}
    out = Dict{String,Float64}()
    for b in BAND_ORDER
        col = Symbol(b)
...
function _index_row(store::PSDStore, channel::String)::Dict{String,Float64}
...
function save_psd_png(
    ...
    f = store.freqs[mask]
    y_ch = copy(store.psd[channel][mask])
    y_mu = copy(store.mean_psd[mask])
    # Evitar log(0)
    y_ch .= max.(y_ch, 1e-12)
    y_mu .= max.(y_mu, 1e-12)

    fig = Figure(size = (1100, 540), fontsize = 13)
    ylab = yscale == "log10" ? "log₁₀ PSD (µV²/Hz)" : "PSD (µV²/Hz)"
    ax = Axis(fig[1, 1];
        title = "PSD post-ICA (epochs) — $SUBJECT/$SESSION/$TASK_ID · $channel" *
    ...
    # Marcadores 50 / 100 Hz si entran en el rango
    if fmax >= 50
    ...
    # Tabla de potencias bajo el plot
    bp = _band_row(store, channel)
    pct = _rel_pct(bp)
    hdr = join([b for b in BAND_ORDER if haskey(bp, b)], "   |   ")
    ...
    out = joinpath(HERE, "psd_$(channel)_fmax$(round(Int, fmax)).png")
...
function handle_request(sock, store::PSDStore)
    ...
    try
        ...
        elseif method == "GET" && path_only == "/api/meta"
            ...
            body = "{\"channels\":[" * join(["\"$c\"" for c in store.channels], ",") * "]," *
                   ...
                   "\"n_channels\":$(length(store.channels)),\"bands\":[$bands_j]}"
        ...
        elseif method == "GET" && path_only == "/api/psd"
            ...
            haskey(store.psd, ch) || error("Canal desconocido: $ch")
            ...
            # Decimar si hay muchos puntos
            if length(mask) > 800
                step = ceil(Int, length(mask) / 800)
                mask = mask[1:step:end]
            end
            f = store.freqs[mask]
            ych = store.psd[ch][mask]
            ymu = store.mean_psd[mask]

            bp = _band_row(store, ch)
            bp_mu = _mean_band_power(store)
            pct = _rel_pct(bp)
            pct_mu = _rel_pct(bp_mu)
            idx = _index_row(store, ch)
            ...
            body = "{\"channel\":\"$ch\"," *
                   ...
                   "\"psd\":[" * join(round.(ych; digits=8), ",") * "]," *
                   "\"psd_mean\":[" * join(round.(ymu; digits=8), ",") * "]," *
        ...
        elseif method == "POST" && path_only == "/api/save"
            ...
            req = _parse_json(raw)
            out = save_psd_png(store, req.channel, req.fmax, req.yscale, req.show_mean)
...
function html_page(store::PSDStore)::String
...
<head>
<meta charset="utf-8"/>
<title>NeuroMIND — PSD espectral (canal + promedio)</title>
<style>
  :root {
...
<body>
<header>
  <h1>PSD espectral — canal + promedio</h1>
  <span>$SUBJECT / $SESSION / task-$TASK_ID · fs=$(round(Int, store.fs)) Hz · $(length(store.channels)) canales</span>
</header>
<div class="wrap">
  <div class="card">
    <div class="src">
      <strong>Fuente de datos (no es señal cruda).</strong>
      Curvas: <code>tables/psd_by_channel.csv</code> —
      PSD FFT Hamming sobre epochs válidos tras filtrado + ICA + AR (paso [6/8]).
      Potencias: <code>band_power_summary.csv</code>.
      Índices: <code>spectral_indices.csv</code>.
      Los valles marcados a 50/100 Hz corresponden al <em>notch</em> de red del preprocesado.
    </div>
    <div class="toolbar">
      <div class="field">
        <label>Canal</label>
      ...
      <div class="field">
        <label>Eje Y</label>
        <select id="yscale">
          <option value="log10" selected>log₁₀ PSD</option>
          <option value="linear">PSD lineal</option>
        </select>
      </div>
  ...
  <div class="card">
    <strong style="font-size:13px">Potencia por bandas (µV²)</strong>
    <p style="margin:4px 0 0;font-size:12px;color:var(--muted)">Valores de <code>band_power_summary.csv</code> · % relativo a la suma de bandas NeuroMIND</p>
...
async function refreshPlot() {
  ...
  setStatus('Cargando…');
  const url = '/api/psd?channel=' + encodeURIComponent(channel) + '&fmax=' + fmax;
  const res = await fetch(url);
  const data = await res.json();
  if (!res.ok) { setStatus(data.error || 'Error', 'err'); return; }
  _last = { data, fmax, yscale, showMean };
  drawPSD(data, fmax, yscale, showMean);
  fillTable(data);
  setStatus(channel + ' · 0.5–' + fmax + ' Hz · fuente: psd_by_channel.csv (post-ICA/AR)');
}
...
function fillTable(data) {
  ...
  const row = (label, abs, pct) => '<tr><td><strong>' + label + '</strong></td>' +
    names.map(n => {
      ...
      return '<td>' + a.toFixed(3) + '<br><span style="color:#64748b">' + p.toFixed(1) + '%</span></td>';
    }).join('') + '</tr>';

  tbody.innerHTML =
    row(data.channel + ' (µV²)', data.band_power, data.band_pct) +
    row('Promedio (µV²)', data.band_power_mean, data.band_pct_mean);
...
function drawPSD(data, fmax, yscale, showMean) {
  ...
  const f = data.f || [];
  let yCh = (data.psd || []).map(v => Math.max(v, 1e-12));
  let yMu = (data.psd_mean || []).map(v => Math.max(v, 1e-12));
  if (yscale === 'log10') {
  ...
  // Sombras de banda (sin texto — leyenda HTML encima del canvas)
  BANDS.forEach(b => {
  ...
  if (showMean) {
    ...
    ctx.fillText('Promedio (' + $(length(store.channels)) + ' ch)', lx + 24, ly + 4);
    lx += 150;
  }
  ...
  ctx.fillText('PSD post-ICA (epochs) — sub-M05/ses-T2/EC · fs=' + FS.toFixed(1) + ' Hz',
             pad.l + pw/2, 18);
  ...
  ctx.fillText(yscale === 'log10' ? 'log₁₀ PSD (µV²/Hz)' : 'PSD (µV²/Hz)', 0, 0);
  ctx.restore();
}
...
async function savePng() {
  ...
  try {
    const res = await fetch('/api/save', {
      ...
      body: JSON.stringify({channel, fmax, yscale, show_mean})
    });
    const data = await res.json();
...
window.addEventListener('resize', () => {
  clearTimeout(_rt);
  _rt = setTimeout(() => {
    if (_last) drawPSD(_last.data, _last.fmax, _last.yscale, _last.showMean);
  }, 120);
});
...
function main()
    println("Cargando tablas PSD espectral desde $TABLES …")
    store = load_store()

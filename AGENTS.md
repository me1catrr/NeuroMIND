...
| **dwPLI** | Debiased wPLI² estimator (Vinck 2011); recommended for group comparisons |
...
- Mixing matrix: `A = pinv(W_total)` — required because n_comp < n_channels

### wPLI — multi-method architecture (since 2026-05-26)
Configured in `config/batch_pipeline.toml`:
```toml
[connectivity]
wpli_method = "hilbert"      # Butterworth + Hilbert analytic signal (fastest)
wpli_method = "fourier_csd"  # Cross-power spectrum FFT + Hann window
wpli_method = "multitaper"   # DPSS multitaper (MNE standard, best variance-bias tradeoff)
use_dwpli   = true           # debiased wPLI — recommended for group comparisons
```
...
| `config/single_subject.toml` | Parameters for `run_single_subject.jl` |
| `config/batch_pipeline.toml` | Parameters for `run_batch_pipeline.jl` |
...
**These files never go to the repo** (`.gitignore` via `/results/`).

---

## 7. Signal quality policy (QC v2 — 2026-05-24)

### Main montage: 30 channels
Fp2 excluded in **all** subjects/sessions/conditions. It appears problematic in 46% of the dataset.
Produces wPLI matrices of **30 × 30** with **435 edges per band**.
For sensitivity analysis: `exclude_fp2 = false` in `[montage]`.
...
With `segment_length_seconds = 2.0` and `min_cycles_for_wpli = 4.0`:
- DELTA (0.5 Hz × 2s = 1.0 cycles) → below threshold, `@warn`
- THETA (4.0 Hz × 2s = 8.0 cycles) → OK
